...
import FlashCardArea from "./FlashCardArea";
...
const FlashCardPanel = ({
  ...
  const [flashcardFetched, setFlashcardFetched] = useState(false);
  const [input, setInput] = useState("");
  const flashcardMutation = useMutation({
    mutationFn: async (keywords: string) => {
      const parsedkeywords = keywords.split(",");
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/flashcard/${chat_id}/`,
        {
      ...
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-primary_grey rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full relative">
          ...
          <div className="bg-primary_grey  px-4 pb-4 sm:p-10 sm:pb-4">
            {!flashcardFetched && (
              <div className="space-y-3">
                <h3 className="text-center text-2xl font-medium">
                  Enter the section titles for generating Flash cards
                </h3>
                <input
                ...
                <button
                  ...
                  onClick={() => flashcardMutation.mutateAsync(input)}
            ...
            {flashcardFetched && (
              <FlashCardArea data={flashcardMutation.data} />
            )}
          </div>
        </div>
      </div>
